...
def update_text(txt):
    """
    Function to print out text but instead of
    starting a new line it will overwrite the existing line
    """
...
def convert_to_euler(pose_mat):
    """
    Convert the standard 3x4 position/rotation matrix to a x,y,z
    location and the appropriate Euler angles (in degrees)
    """
    yaw = 180 / math.pi * math.atan(pose_mat[1][0] / pose_mat[0][0])
    pitch = 180 / math.pi * math.atan(-1 * pose_mat[2][0] / math.sqrt(
        pow(pose_mat[2][1], 2) + math.pow(pose_mat[2][2], 2)))
...
def convert_to_quaternion(pose_mat):
    """
    Convert the standard 3x4 position/rotation matrix to a x,y,z
    location and the appropriate Quaternion
    """
    # Per issue #2, adding a abs() so that sqrt only results in real numbers
    r_w = math.sqrt(
        abs(1 + pose_mat[0][0] + pose_mat[1][1] + pose_mat[2][2])) / 2
...
class pose_sample_buffer():
    """
    Define a class to make it easy to append pose matricies and convert
    to both Euler and Quaternion for plotting
    """
    ...
    def append(self, pose_mat, t):
        ...
        self.yaw.append(
            180 / math.pi * math.atan(pose_mat[1][0] / pose_mat[0][0]))
        self.pitch.append(180 / math.pi * math.atan(-1 * pose_mat[2][0] / math.sqrt(
            pow(pose_mat[2][1], 2) + math.pow(pose_mat[2][2], 2))))
        self.roll.append(
            180 / math.pi * math.atan(pose_mat[2][1] / pose_mat[2][2]))
        r_w = math.sqrt(
            abs(1 + pose_mat[0][0] + pose_mat[1][1] + pose_mat[2][2])) / 2
        self.r_w.append(r_w)
...
class vr_tracking_reference(vr_tracked_device):
    ...
    def sample(self, num_samples, sample_rate):
        logging.info(
            "Warning: Tracking References do not move, sample isn't much use...")
...
class triad_openvr():
    def __init__(self):
        ...
        self.object_names = {"Tracking Reference": [],
        ...
        for i in range(openvr.k_unMaxTrackedDeviceCount):
            if poses[i].bPoseIsValid:
                ...
                if (device_class == openvr.TrackedDeviceClass_Controller):
                    device_name = "controller_" + \
                        str(len(self.object_names["Controller"]) + 1)
                    ...
                    device_name = "hmd_" + \
                        str(len(self.object_names["HMD"]) + 1)
                    self.object_names["HMD"].append(device_name)
                    self.devices[device_name] = vr_tracked_device(
                        self.vr, i, "HMD")
                elif (device_class == openvr.TrackedDeviceClass_GenericTracker):
                    device_name = "tracker_" + \
                        str(len(self.object_names["Tracker"]) + 1)
                    self.object_names["Tracker"].append(device_name)
                    self.devices[device_name] = vr_tracked_device(
                        self.vr, i, "Tracker")
                elif (device_class == openvr.TrackedDeviceClass_TrackingReference):
                    device_name = "tracking_reference_" + \
                        str(len(self.object_names["Tracking Reference"]) + 1)
    ...
    def rename_device(self, old_device_name, new_device_name):
        ...
        for i in range(len(self.object_names[self.devices[new_device_name].device_class])):
            if self.object_names[self.devices[new_device_name].device_class][i] == old_device_name:
                self.object_names[self.devices[new_device_name]
                                  .device_class][i] = new_device_name
    ...
    def print_discovered_objects(self):
        for device_type in self.object_names:
            ...
            if len(self.object_names[device_type]) != 1:
                plural += "s"
            logging.info(
                "Found " + str(len(self.object_names[device_type])) + " " + plural)
            for device in self.object_names[device_type]:
                if device_type == "Tracking Reference":
                    logging.info("  " + device + " (" + self.devices[device].get_serial() +
